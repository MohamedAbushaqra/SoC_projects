/* Abushaqra - Jun - 15 - 2024 - created */
/* This Kernel Level driver enables the control of the simple led toggler ip core in Vivado */
...
static int pl_leds_probe(struct platform_device *pdev)
{
	...
	if (!lp)
	{
		dev_err(dev, "Cound not allocate PL-LED device\n");
		return -ENOMEM;
	}
	...
	if (!lp->base_addr)
	{
		dev_err(dev, "PL-LED: Could not allocate iomem\n");
		rc = -EIO;
		goto error2;
	}

	dev_info(dev,"PL-Led at 0x%08x mapped to 0x%08x \n",
